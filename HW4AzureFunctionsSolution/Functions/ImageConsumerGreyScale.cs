...
namespace HW4AzureFunctionsSolution
{
    public static class ImageConsumerGreyScale
    {
        ...
        /// <summary>
        /// HW:4 #1
        /// Converts uploaded images in the converttogreyscale blob container to grey scale
                                    ...
                                    string uploadedBlobUri)
        {
            ...
            try
            {
                await UpdateJobTableWithStatus(log, jobId, status: 2, message: "Image Being Converted", conversionMode: "GreyScale", uploadedBlobUri);
        ...
        /// <returns></returns>
        private static async Task UpdateJobTableWithStatus(ILogger log, string jobId, int status, string message, string conversionMode, string blobUri)
        {
            JobTable jobTable = new JobTable(log, ConfigurationSettings.IMAGEJOBS_PARTITIONKEY);
            await jobTable.InsertOrReplaceJobEntity(jobId, status: status, message: message, conversionMode: conversionMode, blobUri);
        }
    }
}
